...
async def delete_recipe(request: Request, recipe_id: int):
    ...
    # rendered with the deleted recipe's id baked in. Title is URL-encoded
    # so `&`, `=`, `\r\n`, etc. in the recipe name don't corrupt the query
    # string or trip Starlette's CRLF guard in the response header.
    resp = Response(status_code=200)
    resp.headers["HX-Redirect"] = (
        f"/app/?undo={recipe_id}&undo_title={quote(row['title'], safe='')}"
    )
    return resp
